...
ALLOWLIST_FILE="${RIELA_VENDOR_ALLOWLIST:-/opt/riela-sandbox/allowlist.txt}"
...
install_firewall() {
  if ! command -v iptables >/dev/null 2>&1; then
    echo "riela sandbox: iptables is unavailable; refusing firewall mode" >&2
    return 1
  fi
...
if [ "${RIELA_SANDBOX_FIREWALL:-1}" = "1" ]; then
  if ! install_firewall; then
    cat >&2 <<'MSG'
riela sandbox: failed to install firewall rules.
...
MSG
  ...
  echo "riela sandbox: firewall disabled by RIELA_SANDBOX_FIREWALL=0" >&2
fi
